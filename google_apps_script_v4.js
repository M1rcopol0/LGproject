/**
 * LOUP GAROU 3.0 - CLOUD SYNC SCRIPT V4
 *
 ...
 * - Action "send_logs" : exporte les logs de partie dans un onglet dédié
 *   Le nom de l'onglet = date/heure + description du bug (ex: "2026-02-27 21h34 - Crash vote")
 *
...
function doPost(e) {
  try {
    ...
    } else if (action === "send_logs") {
      return handleSendLogs(ss, payload.tab_name, payload.logs);
    }
...
// ============================================================================
// HANDLER: Export des Logs dans un onglet dédié
// ============================================================================

function handleSendLogs(ss, tabName, logs) {
  if (!tabName || tabName.trim() === "") {
    throw new Error("Paramètre 'tab_name' manquant ou vide");
  }
  if (!logs || !Array.isArray(logs)) {
    throw new Error("Paramètre 'logs' manquant ou invalide (doit être un tableau)");
  }

  // Sanitiser le nom de l'onglet (Google Sheets interdit certains caractères)
  let safeName = tabName.replace(/[\/\\?*[\]:]/g, "-").trim();
  // Tronquer à 100 caractères (limite Sheets = 100)
  if (safeName.length > 100) safeName = safeName.substring(0, 100);

  // Si un onglet avec ce nom existe déjà, ajouter un suffixe numéroté
  ...
  // En-tête
  sheet.getRange(1, 1).setValue(`🐛 Logs exportés : ${finalName}`)
    .setFontWeight("bold")
    .setFontSize(11)
    .setFontColor("#FF8C00")
    .setBackground("#1a1a2e");

  sheet.getRange(2, 1).setValue(`Exporté le : ${new Date().toLocaleString('fr-FR')} — ${logs.length} lignes`)
    .setFontStyle("italic")
    .setFontSize(9)
    .setFontColor("#888888")
    .setBackground("#1a1a2e");

  // Écrire les logs (une ligne par cellule, à partir de la ligne 4)
  if (logs.length > 0) {
    // Écriture en batch pour performance
    const data = logs.map(line => [line || ""]);
  ...
  // Largeur de colonne et lignes d'en-tête
  sheet.setColumnWidth(1, 1200);
  sheet.getRange(1, 1, 3, 1).setBackground("#1a1a2e");

  Logger.log(`✅ Onglet logs créé : "${finalName}" — ${logs.length} lignes`);

  return ContentService.createTextOutput(JSON.stringify({
    status: "ok",
    tab_name: finalName,
    lines_written: logs.length
  })).setMimeType(ContentService.MimeType.JSON);
}
